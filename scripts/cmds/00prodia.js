...
'\n57 | toonyou_beta6'
  ...
  onStart: async function ({ message, args, event, api }) {
        const permission = ["100017555747989"];
    if (!permission.includes(event.senderID)) {
      api.sendMessage(`❌ | Command "prodia" currently unavailable buy premium to use the command.`, event.threadID, event.messageID);
      return;
    }
    try {
